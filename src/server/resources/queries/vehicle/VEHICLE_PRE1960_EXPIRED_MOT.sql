...
where t.status_id = 6 -- only consider passed tests
and t.expiry_date < CURRENT_DATE -- that have expired
and t.id = (
...
and veh.registration not like "%-%" -- exclude registration with hyphen - these are likely foreign plates
