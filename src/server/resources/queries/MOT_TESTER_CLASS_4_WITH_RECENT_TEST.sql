select distinct(p.username) as username, s.name as site, v.registration as registration, mtc.odometer_value as mileage
...
and mtc.id = (select max(lmtc.id) from mot_test_current lmtc where lmtc.person_id = p.id) -- Checking only the latest test for user
