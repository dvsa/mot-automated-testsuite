...
WHERE veh.vin IS NOT NULL
  ...
  AND mtc.submitted_date =
  (SELECT MAX(mtc1.submitted_date) FROM mot_test_current mtc1 WHERE mtc1.vehicle_id = mtc.vehicle_id AND mtc1.submitted_date > '2018-08-01' -- find more recent MOTs with connected certificates)
