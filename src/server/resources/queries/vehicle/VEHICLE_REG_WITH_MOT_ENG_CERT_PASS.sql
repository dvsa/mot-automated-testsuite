select veh.registration, concat(ma.name, ' ', mo.name) as make, DATE_FORMAT(veh.manufacture_date, "%e %M %Y"), DATE_FORMAT(mtc.expiry_date, "%e %M %Y"), concat(left(mtc.number,4), ' ', substring(mtc.number,5,4), ' ', right(mtc.number,4)) as testno, dvla.recent_v5_document_number
from vehicle veh, model_detail md, model mo, make ma,
  (select max(id) as id, vehicle_id  from mot_test_current
   ...
   mot_test_current mtc, dvla_vehicle dvla
...
and mtc.expiry_date > curdate() -- mot valid
limit 10