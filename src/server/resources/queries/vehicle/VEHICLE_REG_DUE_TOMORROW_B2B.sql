select veh.registration, ma.name make, mo.name model, DATE_FORMAT(veh.first_registration_date, "%Y-%m-%d") first_used_date, DATE_FORMAT(mtc.expiry_date, "%Y-%m-%d") expiry_date, mtc.number
...
and datediff(mtc.expiry_date, curdate()) = 1 -- mot due tomorrow
limit 10