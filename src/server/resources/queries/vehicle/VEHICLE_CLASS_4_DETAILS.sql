-- The SQL is to to find the most recent Odometer value, issue date and details for a class 4 vehicle
SELECT
       veh.registration,
       veh.vin,
       mtc.odometer_value,
       clp.name as colour1,
       cls.name as colour2,
       date_format(mtc.issued_date, "%e %M %Y") as issue_date
  FROM vehicle veh
 ...
 JOIN colour_lookup clp ON clp.id = veh.primary_colour_id
 JOIN colour_lookup cls ON cls.id = veh.secondary_colour_id
