@tradeapi
Feature: 01 - Searching by test number

  Scenario: A Trade API user searches MOT details by MOT test number
    Given I load "VEHICLE_REG_MOT_EXPIRED_B2B" as {registration}, {make}, {model}, {first_used_date}, {expiry_date}, {number}
    And I invoke the Trade API search by test number with {number}
