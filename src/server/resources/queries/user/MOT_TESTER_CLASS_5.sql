...
FROM
  ...
  JOIN (
    SELECT id, name
     FROM site
-- Approved site
     WHERE site_status_id = '1'
-- select sites that can complete 2 and 3 axle tests
     AND default_service_brake_test_class_3_and_above_id in (3,4,5)
       ) s
...
WHERE
  ...
  -- Tester authorised to test class 5 vehicles
  AND aftm.vehicle_class_id = 5
  -- Site authorised to test class 5 vehicles
  AND afts.vehicle_class_id = 5
